...
import {
  DownloadVelocityOsJournalParams,
  UnlockVelocityOsJournalBody,
  type ErrorResponse,
} from "@workspace/api-zod";
import { db } from "@workspace/db";
import { velocityOsJournalLeadsTable } from "@workspace/db/schema";
import {
  fetchVelocityOsJournal,
  isVelocityOsJournalAvailable,
} from "../lib/velocityOsJournalStorage";

const router: IRouter = Router();

const DOCUMENT_ID = "operators-daily-journal";
const DOCUMENT_VERSION = "first-edition-2026";
const DOCUMENT_TITLE = "The Operator's Daily Journal";
const DOCUMENT_FILENAME = "velocity-os-operators-daily-journal.pdf";
...
router.post("/velocity-os/journal-unlocks", async (req, res) => {
  const body =
    req.body && typeof req.body === "object" && !Array.isArray(req.body)
      ...
      : req.body;
  const parsed = UnlockVelocityOsJournalBody.safeParse(body);
  ...
  let available = false;
  try {
    available = await isVelocityOsJournalAvailable();
  } catch {
    available = false;
  }
  ...
  if (!available) {
    res
      ...
      .json(
        error(
          "The journal file is temporarily unavailable. Please try again in a few minutes.",
        ),
      );
    return;
  }
  ...
  try {
    ...
    res.status(201).json({
      success: true,
      message:
        "Your journal is ready. This private download link expires in 10 minutes.",
      downloadUrl: `/api/velocity-os/journal-downloads/${token}`,
...
router.get("/velocity-os/journal-downloads/:token", async (req, res) => {
  const parsed = DownloadVelocityOsJournalParams.safeParse(req.params);
  if (!parsed.success) {
    res
      .status(401)
      .json(
  ...
  try {
    journalResponse = await fetchVelocityOsJournal(req.get("range"));
  } catch {
    res
      .status(503)
      .json(
        error(
          "The journal download is temporarily unavailable. Your link is still valid, so please try again.",
        ),
      );
    return;
  }

  if (!journalResponse.ok || !journalResponse.body) {
    res
      .status(503)
      .json(
        error(
  ...
  res.status(journalResponse.status);
  ...
  for (const header of [
    ...
    const value = journalResponse.headers.get(header);
    if (value) {
  ...
  const stream = Readable.fromWeb(
    journalResponse.body as ReadableStream<Uint8Array>,
  );
